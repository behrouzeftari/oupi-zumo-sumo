...
# SIGNATURE -> favoured combat maneuver when breaking a stalemate. Styles:
#   0 FLANKER         -- reverse, big pivot, angled burst into the side.
#   1 CHARGER         -- short reverse run-up, then a long full-power head-on ram.
#   2 COUNTER-PUNCHER -- give ground, then a pivot-shove into the lunging side.
...
def handle_combat_maneuver():
    ...
    if maneuver_style == 0:
        # FLANKER -- circle to the side: brief reverse, a big ~90-degree pivot,
        # then an angled burst into the opponent's flank where it can't push
        # back. The large turn is what gets it off the opponent's nose.
        flank_turn_ms = MANEUVER_TURN_MS + 150
        if elapsed < MANEUVER_BACKUP_MS:
            drive(-backup_speed, -backup_speed)
            return
        elif elapsed < MANEUVER_BACKUP_MS + flank_turn_ms:
            drive(turn_speed * maneuver_turn_dir,
                  -turn_speed * maneuver_turn_dir)
            return
        elif elapsed < MANEUVER_BACKUP_MS + flank_turn_ms + MANEUVER_BURST_MS:
            if maneuver_turn_dir > 0:
                drive(burst_speed, burst_speed * 55 // 100)
            else:
                drive(burst_speed * 55 // 100, burst_speed)
            return

    elif maneuver_style == 1:
        # CHARGER -- straight-line bully: a short hard reverse for a run-up,
        # then a long full-power head-on ram. No turning away -- it just bets
        # on raw power and re-rams.
        ram_speed = attack_speed * 120 // 100
        charge_burst_ms = MANEUVER_BURST_MS + 200
        if elapsed < MANEUVER_BACKUP_MS:
            drive(-backup_speed, -backup_speed)
            return
        elif elapsed < MANEUVER_BACKUP_MS + charge_burst_ms:
            drive(ram_speed, ram_speed)
            return

    elif maneuver_style == 2:
        # COUNTER-PUNCHER -- absorb then redirect: give ground (a longer
        # reverse) to draw the opponent forward, then a hard pivot-shove with
        # one tread forward and the other reversed, spinning into its side as
        # it lunges -- using the opponent's own momentum against it.
        absorb_ms = MANEUVER_BACKUP_MS + 150
        pivot_ms = MANEUVER_BURST_MS + 100
        if elapsed < absorb_ms:
            drive(-backup_speed, -backup_speed)
            return
        elif elapsed < absorb_ms + pivot_ms:
            if maneuver_turn_dir > 0:
                drive(burst_speed, -turn_speed)
            else:
                drive(-turn_speed, burst_speed)
            return
